...
private static function build_student_data( $user, $course_id, $order_id, $access_type, $membership_id = null ) {
    ...
    // Get last activity
    $last_activity = get_user_meta( $user->ID, '_llms_last_user_activity', true );
...
private static function render_student_row( $student, $course_id ) {
    ...
    if ( $student['last_activity'] ) {
        echo esc_html( date_i18n( 'M j, Y', strtotime( $student['last_activity'] ) ) );
    } else {
        echo '<span class="llmsgaa-no-activity">No activity</span>';
    }
    echo '</td>';
