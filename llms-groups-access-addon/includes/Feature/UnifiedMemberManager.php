...
public static function get_all_group_members( $group_id ) {
    ...
    // Get active members from lifterlms_user_postmeta
    $active_members = $wpdb->get_results( $wpdb->prepare(
        "SELECT DISTINCT user_id, meta_value as role
...
/**
 * Add a new member to the group - fixed for existing users
 * 
 * @param int $group_id
 * @param string $email
...
public static function add_member( $group_id, $email, $role = 'member' ) {
    ...
    if ( $user ) {
        ...
        // User exists but not in group: add them directly using the same pattern as your system
        try {
            $result = $wpdb->insert(
            ...
            if ( $result === false ) {
                error_log( "❌ Failed to insert group role for user {$user->ID} in group {$group_id}" );
                return new \WP_Error( 'database_error', 'Failed to add user to group' );
            }
            
            error_log( "✅ Added existing user {$email} (ID: {$user->ID}) to group {$group_id} as {$role}" );
            return true;
            
        } catch ( Exception $e ) {
            error_log( "❌ Exception adding user to group: " . $e->getMessage() );
            return new \WP_Error( 'database_error', 'Database error: ' . $e->getMessage() );
        }
        
    } else {
        // User doesn't exist - send invitation using your existing service
        ...
        error_log( "📧 Sent invitation to new user {$email} for group {$group_id} as {$role}" );
        return true;
    }
}
...
public static function assign_license( $order_id, $email ) {
    ...
    if ( $user ) {
        ...
        // Your existing system will handle enrollment scheduling via the post meta update hooks
        ...
        error_log( "✅ License {$order_id} assigned to user {$user->ID} ({$email}) - existing system will handle scheduling" );
    } else {
        ...
        error_log( "📧 License {$order_id} assigned to pending user ({$email})" );
    }
...
// Add this method to your UnifiedMemberManager class
...
public static function remove_member( $group_id, $user_id = 0, $email = '' ) {
    ...
    try {
        ...
        if ( $user_id ) {
            ...
            error_log( "🗂 Removed group role: {$role_delete} records deleted" );
        }
        ...
        error_log( "📧 Removed invitations: {$invite_delete} records deleted" );
        ...
        // 4. Unassign each license (your existing system will handle unenrollment automatically)
        $unassigned_count = 0;
        ...
        if ( $user_id && function_exists( 'LLMS_Groups_Members' ) ) {
            try {
                ...
                error_log( "🏫 LifterLMS group removal result: " . var_export( $llms_removal, true ) );
            } catch ( Exception $e ) {
                error_log( "⚠️ LifterLMS group removal failed: " . $e->getMessage() );
            }
        }
