...
<!-- Your Orders Box (Keep existing functionality) -->
<!-- Your Orders Box (Keep existing functionality) -->
<!-- Your Orders Box (Keep existing functionality) -->
<div class="llmsgaa-box">
  ...
  <div id="llmsgaa-redeem-modal" class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div style="border:2px solid #666; border-radius:10px; background:#f9f9f9; padding:20px; max-width:320px; position:relative;">
      <span class="llmsgaa-modal-close" style="cursor:pointer; position:absolute; top:10px; right:14px; font-size:24px;">&times;</span>
      <h3 class="text-lg font-semibold mb-4">Select Start Date</h3>
      <form id="llmsgaa-redeem-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="space-y-4">
        ...
        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Confirm Start Date</button>
      </form>
    </div>
  </div>
...
<tr class="member-row" data-email="<?php echo esc_attr( $member['email'] ); ?>" data-user-id="<?php echo esc_attr( $member['user_id'] ?: '' ); ?>">
  ...
  <td>
    <span class="status-badge <?php echo $member['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'; ?> px-2 py-1 rounded-full text-xs">
...
jQuery(document).ready(function($) {
    ...
    function showAssignLicenseModal(email, memberName, licenses) {
        ...
        if (licenses.length === 0) {
            ...
            html += '<div style="max-height: 300px; overflow-y: auto; border: 1px solid #ddd; border-radius: 6px; padding: 10px;">';
            
            licenses.forEach(license => {
                html += '<label style="display: block; padding: 12px; border-bottom: 1px solid #eee; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background=\'#f8f9fa\'" onmouseout="this.style.background=\'transparent\'">';
                html += `<input type="checkbox" value="${license.ID}" class="license-checkbox" style="margin-right: 10px; transform: scale(1.2);">`;
                html += `<strong>${license.course_title}</strong><br>`;
                html += `<small style="color: #666;">🗓 Start: ${license.start_date_formatted || 'Not set'}</small>`;
                html += '</label>';
            });
            ...
            html += `<button onclick="assignSelectedLicenses('${email}')" class="modal-btn-primary">🎫 Assign Selected</button>`;
            html += '</div>';
        }
