...
jQuery(document).ready(function($) {
    ...
    window.updateBulkActionsBar = function() {
        ...
        if (selectedCount > 0) {
            ...
            $('#bulk-assign-licenses-btn').html(`🎫 Assign Licenses (${selectedCount})`);
        } else {
    ...
    window.showBulkAssignModal = function(selectedEmails) {
        ...
        const groupId = window.llmsgaa_group_id || window.groupId;
        if (!groupId) {
            alert('Error: Could not determine group ID. Please refresh the page and try again.');
            return;
        }
        
        // Show loading state
        const loadingHTML = `
            <div id="llmsgaa-loading-modal" style="
                position: fixed;
                top: 0;
                left: 0;
                right: 0;
        ...
        // Fetch available group orders
        $.post(ajaxurl || '/wp-admin/admin-ajax.php', {
            action: 'llmsgaa_get_available_group_orders',
            group_id: groupId,
            nonce: window.llmsgaa_nonce || window.ajaxNonce
        })
        .done(function(response) {
            $('#llmsgaa-loading-modal').remove();
            if (response.success && response.data) {
                buildBulkModal(selectedEmails, response.data);
            } else {
                buildBulkModal(selectedEmails, []);
            }
        })
        .fail(function() {
            $('#llmsgaa-loading-modal').remove();
    ...
    // Build bulk modal
    window.buildBulkModal = function(selectedEmails, orderData) {
        ...
        if (orderData && Array.isArray(orderData)) {
            availableOrders = orderData.filter(order => 
                !order.assigned || order.status === 'pending' || order.status === 'available'
            );
        }
        
        availableOrders.forEach(order => {
            const courseKey = order.course || order.product_title || 'Unknown Course';
            const dateKey = order.start_date || 'Flexible Start';
            const groupKey = `${courseKey}|${dateKey}`;
        ...
        const willBeAssigned = Math.min(memberCount, totalAvailable);
        const willNotBeAssigned = Math.max(0, memberCount - totalAvailable);
        
        let modalContent = createModalHeader();
        modalContent += createAssignmentSummary(memberCount, willBeAssigned, willNotBeAssigned, totalAvailable);
        modalContent += createLicenseGroups(groupedOrders, memberCount, totalAvailable);
        modalContent += createMemberList(selectedEmails, willBeAssigned);
        modalContent += createModalFooter(totalAvailable, willBeAssigned, selectedEmails);
        
        const modalHTML = `
            <div id="llmsgaa-bulk-modal" style="
                position: fixed;
                top: 0;
                left: 0;
                right: 0;
    ...
    window.createAssignmentSummary = function(memberCount, willBeAssigned, willNotBeAssigned, totalAvailable) {
        if (totalAvailable === 0) {
            return `
                <div style="
                    ...
                    margin-bottom: 20px;
                ">
                    <strong>⚠️ No Available Licenses</strong><br>
                    All licenses for this group have been assigned.
                </div>
            `;
        }
        ...
        if (willBeAssigned === memberCount) {
            return `
                ...
                ">
                    ...
                    All ${memberCount} selected member(s) will receive licenses.
                </div>
            `;
        }
        ...
        if (willBeAssigned < memberCount) {
            return `
                <div style="
                    ...
                    margin-bottom: 20px;
                ">
                    <strong>⚠️ Limited Licenses Available</strong><br>
                    • ${willBeAssigned} member(s) will receive licenses<br>
                    • ${willNotBeAssigned} member(s) will NOT receive licenses (not enough available)<br>
                    <small style="margin-top: 5px; display: block;">
                        Licenses will be assigned in the order members are listed.
                    </small>
                </div>
            `;
        }
        ...
        return `
            <div style="
                ...
                margin-bottom: 20px;
            ">
                <strong>ℹ️ More Licenses Than Members</strong><br>
                All ${memberCount} selected member(s) will receive licenses.<br>
                ${totalAvailable - memberCount} license(s) will remain available.
            </div>
        `;
    };
        ...
        return `
            <div style="margin-bottom: 20px;">
                <h3 style="margin: 0 0 12px 0; font-size: 16px; color: #333;">
                    Available Licenses (${totalAvailable} total)
                </h3>
    ...
    window.processBulkAssignment = function(encodedEmails) {
        const selectedEmails = JSON.parse(atob(encodedEmails));
        const groupId = window.llmsgaa_group_id || window.groupId;
        const nonce = window.llmsgaa_nonce || window.ajaxNonce;
        
        $('#llmsgaa-bulk-modal button').prop('disabled', true);
        $('#llmsgaa-bulk-modal button:last').html('⏳ Processing...');
        
        $.post(ajaxurl || '/wp-admin/admin-ajax.php', {
            action: 'llmsgaa_bulk_assign_auto',
            ...
            if (response.success) {
                $('#llmsgaa-bulk-modal > div').html(`
                    <div style="text-align: center; padding: 40px;">
                        ...
                        <p>${response.data.message || response.data || 'Licenses assigned successfully!'}</p>
                ...
                alert('Error: ' + (response.data || 'Assignment failed'));
                $('#llmsgaa-bulk-modal button').prop('disabled', false);
                $('#llmsgaa-bulk-modal button:last').html('Retry Assignment');
            }
        })
        .fail(function() {
